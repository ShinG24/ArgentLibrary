...
namespace argent::graphics::dx12
{
	BottomLevelAccelerationStructure::BottomLevelAccelerationStructure(const GraphicsDevice* graphics_device,
		const GraphicsCommandList* graphics_command_list, const BLASBuildDesc* build_desc, bool is_triangle):
		unique_id_(AccelerationStructureManager::GenerateBlasUniqueId())
	{

		std::vector<D3D12_RAYTRACING_GEOMETRY_DESC> geometry_desc(build_desc->vertex_buffer_vec_.size());

		//頂点バッファの数とインデックスバッファの数が違うかつ、
		//インデックスバッファが0個ではないときはサポートしてない
		...
		for(size_t i = 0; i < geometry_desc.size(); ++i)
		{
			...
			const auto& vertex_buffer = build_desc->vertex_buffer_vec_.at(i);

			//フラグは基本これでいい
			desc.Flags = D3D12_RAYTRACING_GEOMETRY_FLAG_OPAQUE;

			//当たり判定用に使う Intersection Shader があるかないか
			...
			if(!is_triangle)
			{
				desc.AABBs.AABBs.StartAddress = vertex_buffer->GetView().BufferLocation;
				desc.AABBs.AABBs.StrideInBytes = vertex_buffer->GetView().StrideInBytes;
				//TODO 複数個のAABBに対応できるようにする
				desc.AABBs.AABBCount = 1u;	
			}
			else
			{
				desc.Triangles.VertexBuffer.StartAddress =  vertex_buffer->GetView().BufferLocation;
				desc.Triangles.VertexBuffer.StrideInBytes = vertex_buffer->GetView().StrideInBytes;
				desc.Triangles.VertexCount = vertex_buffer->GetVertexCounts();
				...
				{
					const auto& index_buffer = build_desc->index_buffer_vec_.at(i);
					desc.Triangles.IndexBuffer = index_buffer->GetView().BufferLocation;
					desc.Triangles.IndexCount = index_buffer->GetIndexCounts();
					desc.Triangles.IndexFormat = DXGI_FORMAT_R32_UINT;
				}
			}
		}

		//デバイスから必要なバッファサイズを取得するために色々準備する


		D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS as_input{};
		as_input.DescsLayout = D3D12_ELEMENTS_LAYOUT_ARRAY;
		//アップデートしないなら圧縮して当たり判定が最速な形に変えるべき
		as_input.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE;
		as_input.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;	//これは固定
		...
		//ConstantBufferと同じサイズにアライメント(256)
		...
		as_desc.Inputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE;	//ここのフラグはInputsで使ったやつと共通
