...
namespace argent::graphics
{
	...
	void FrameResource::Awake(const GraphicsContext* graphics_context, UINT back_buffer_index)
	{
		back_buffer_index_ = back_buffer_index;
		rtv_descriptor_ = graphics_context->rtv_descriptor_->PopDescriptor();
		dsv_descriptor_ = graphics_context->dsv_descriptor_->PopDescriptor();

		graphics_context->swap_chain_->GetBuffer(back_buffer_index_, resource_object_.ReleaseAndGetAddressOf());

		graphics_context->graphics_device_->CreateRTV(resource_object_.Get(), rtv_descriptor_.cpu_handle_);
		...
		hr = graphics_context->graphics_device_->GetDevice()->CreateCommittedResource(&heap_prop, D3D12_HEAP_FLAG_NONE,
			&res_desc, D3D12_RESOURCE_STATE_DEPTH_WRITE, &depth_clear_value,
			IID_PPV_ARGS(depth_buffer_.ReleaseAndGetAddressOf()));
		_ASSERT_EXPR(SUCCEEDED(hr), L"Failed to Create Depth Buffer");
		...
		depth_stencil_view_desc.Flags = D3D12_DSV_FLAG_NONE;
		depth_stencil_view_desc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
		graphics_context->graphics_device_->GetDevice()->CreateDepthStencilView(depth_buffer_.Get(), &depth_stencil_view_desc,
			dsv_descriptor_.cpu_handle_);
